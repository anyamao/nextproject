            ...
            return (
              ...
              >
                ...
                <div className="absolute top-3 right-3 z-20 flex items-center gap-2 pointer-events-none">
                  ...
                  <div
                    className="relative"
                    ref={openMenu === course.id ? menuRef : null}
                  >
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                      ...
                      className="p-2 rounded-full bg-white text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition shadow-sm pointer-events-auto"
                      title="Действия"
                    >
                    ...
                    {openMenu === course.id && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-30 pointer-events-auto">
                        {/* Если курс в избранном — кнопка убрать */}
                        {course.is_favorite && (
                          <button
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              toggleFavorite(e, course.id, course.slug);
                              setOpenMenu(null);
                            }}
                        ...
                        {/* Если курс не записан — кнопка записаться */}
                        {!course.is_enrolled && (
                          <Link
                        ...
                        {/* Если курс записан — кнопка выйти */}
                        {course.is_enrolled && (
                          <button
                            onClick={(e) => {
                              e.preventDefault();
                              ...
                              handleUnenroll(course.id, course.slug);
                            }}
                            disabled={unenrolling === course.id}
                            className="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 disabled:opacity-50 border-t border-gray-100 mt-1 pt-1"
                          >
