...
function ResultsContent({ testId }: { testId: number }) {
  const searchParams = useSearchParams();
  const [result, setResult] = useState<Result | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // ✅ Правильный ключ: используем уже разрешённый testId
    ...
    setLoading(false);
  }, [testId]); // ✅ testId — число, не Promise
  ...
  const returnTo = searchParams.get("returnTo") || `/ege/math-profile`;
...
// 🔹 Главный компонент: разрешает params и передаёт число в ResultsContent
export default async function TestResultsPage({
  ...
  // ✅ Оборачиваем в Suspense, потому что ResultsContent использует useSearchParams
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-blue-50">
