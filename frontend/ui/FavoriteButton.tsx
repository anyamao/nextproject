...
export default function FavoriteButton({
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`rounded-full  ${isFavorite(articleId) ? "bg-purple-200 border-purple-300" : ""} p-[7px] border-[1px] border-gray-400 hover:bg-gray-50 transition relative ${className}`}
      title={
    ...
    >
      {isFavorite(articleId) ? (
        ...
        <Bookmark className="w-[15px] h-[15px] text-gray-700" />
      )}
    </button>
  );
}
