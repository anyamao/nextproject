...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    console.log("🔍 Signup attempt:", {
      ...
      password_repr: formData.password.slice(0, 10) + "...",
    });
    ...
    if (formData.password.length > 36) {
      setError("Пароль слишком длинный (макс. 36 символов)");
      return;
    }
    ...
    try {
      ...
      setSuccess(true);
      toggleRegister();
      const newUser = { email: formData.email, username: formData.username };
      useContactStore.getState().setUser(newUser);
    } catch (err: unknown) {
      const rawMessage =
